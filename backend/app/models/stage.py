...
class ContractorAllocation(Base):
    ...
    id: Mapped[UUID] = mapped_column(GUID(), primary_key=True, default=uuid4)
    stage_summary_id: Mapped[UUID] = mapped_column(GUID(), ForeignKey("stage_summaries.id"), nullable=False)
    stage: Mapped[StageName] = mapped_column(Enum(StageName, name="stage_name"), nullable=False)
    contractor_id: Mapped[UUID] = mapped_column(GUID(), ForeignKey("contractors.id"), nullable=False)
...
class StageProgressEntry(Base):
    ...
    id: Mapped[UUID] = mapped_column(GUID(), primary_key=True, default=uuid4)
    stage_summary_id: Mapped[UUID] = mapped_column(GUID(), ForeignKey("stage_summaries.id"), nullable=False)
    allocation_id: Mapped[Optional[UUID]] = mapped_column(GUID(), ForeignKey("contractor_allocations.id"), nullable=True)
...
class CuttingAnalysis(Base):
    ...
    cutting_supervisor_id: Mapped[Optional[UUID]] = mapped_column(GUID(), ForeignKey("users.id"), nullable=True)
    assigned_to: Mapped[Optional[UUID]] = mapped_column(GUID(), ForeignKey("users.id"), nullable=True)
    responsible_role: Mapped[Optional[str]] = mapped_column(String(80), nullable=True)
    completed_by: Mapped[Optional[UUID]] = mapped_column(GUID(), ForeignKey("users.id"), nullable=True)
    completed_at: Mapped[Optional[datetime]] = mapped_column(DateTime(timezone=True), nullable=True)
...
class MillWastageRecord(Base):
    ...
    cutting_analysis_id: Mapped[Optional[UUID]] = mapped_column(GUID(), ForeignKey("cutting_analysis.id"), nullable=True)
    planned_wastage_m: Mapped[float] = mapped_column(Numeric(14, 3), nullable=False, default=0)
    actual_wastage_m: Mapped[float] = mapped_column(Numeric(14, 3), nullable=False, default=0)
    wastage_difference_m: Mapped[float] = mapped_column(Numeric(14, 3), nullable=False, default=0)
    flag: Mapped[str] = mapped_column(String(16), nullable=False, default="normal")
    recorded_by: Mapped[Optional[UUID]] = mapped_column(GUID(), ForeignKey("users.id"), nullable=True)
...
class PackingOutput(Base):
    ...
    updated_by: Mapped[Optional[UUID]] = mapped_column(GUID(), ForeignKey("users.id"), nullable=True)
    assigned_to: Mapped[Optional[UUID]] = mapped_column(GUID(), ForeignKey("users.id"), nullable=True)
    responsible_role: Mapped[Optional[str]] = mapped_column(String(80), nullable=True)
    completed_by: Mapped[Optional[UUID]] = mapped_column(GUID(), ForeignKey("users.id"), nullable=True)
    completed_at: Mapped[Optional[datetime]] = mapped_column(DateTime(timezone=True), nullable=True)
